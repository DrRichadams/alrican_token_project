...
const AffiliatesReceipt = () => {

    const navigate = useNavigate();
    // const {withdrawRequest, removeWithdrawRequest, user} = UserAuth();
    const {affiliatesRequest, removeAffiliatesRequest, user} = UserAuth();
    console.log("Withdraw", affiliatesRequest)

    if(!affiliatesRequest) return <ClipLoader color={colors.accent} loading={true} cssOverride={override} size={150} />
    ...
    <div>
      ...
      <div>
        <DetailBox>
            ...
            <Detail>USD$ {affiliatesRequest?.amount}</Detail>
        </DetailBox>
        <DetailBox>
            <DetailTitle>Wallet type</DetailTitle>
            <Detail>{affiliatesRequest?.walletType}</Detail>
        </DetailBox>
        <DetailBox>
            <DetailTitle>Withdrawing from</DetailTitle>
            <Detail>{affiliatesRequest?.type}</Detail>
        </DetailBox>
        <DetailBox>
            <DetailTitle>Wallet address</DetailTitle>
            <Detail>{affiliatesRequest?.walletAddress}</Detail>
        </DetailBox>
    </div>
    </div>
  )
...
export default AffiliatesReceipt
